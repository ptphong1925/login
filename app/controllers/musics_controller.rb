class MusicsController < ApplicationController
  ...
  before_action(only: [:show, :edit, :update, :destroy]) { authorize @music}
  after_action(only: [:index]) { authorize @musics }
  after_action(only: [:new, :create]) { authorize @music }
  ...
  private
    ...
    def music_params
      params.require(:music).permit(:title, :content, :catalogue, :poster_id, :poster_type)
    end
end
